...
const VoicePage = () => {
  ...
  const [currentSoundKey, setCurrentSoundKey] = useState(null); // To store the current sound
  ...
  const stopAndPlaySound = async (soundKey) => {
    if (sound) {
  ...
  const handleContinue = () => {
    if (isAnswerCorrect) {
      navigation.navigate("Bottom", { screen: "Levels" });
      resetGame();
    } else {
      reset();
    }
      ...
      <View style={[styles.container, { paddingTop: headerHeight + 10 }]}>
        ...
        <View style={styles.buttonContainer}>
          <TouchableOpacity
            ...
            // Ensure sound does not play during check
